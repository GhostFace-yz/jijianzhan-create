...
import { listCharacterVersions, rollbackCharacter } from '../../api/characters';
import { Button } from '../../components/ui/Button';
import { Modal } from '../../components/ui/Modal';
import type { Character, SnapshotMeta } from '../../types';

interface VersionHistoryProps {
  projectId: string;
  character: Character;
  onRolledBack: (character: Character) => void;
}

export function VersionHistory({ projectId, character, onRolledBack }: VersionHistoryProps) {
  const [rollbackTarget, setRollbackTarget] = useState<SnapshotMeta | null>(null);
  const [isRollingBack, setIsRollingBack] = useState(false);
  const [rollbackError, setRollbackError] = useState<string | null>(null);

  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ['character-versions', projectId, character.id],
    queryFn: () => listCharacterVersions(projectId, character.id),
    enabled: Boolean(projectId) && Boolean(character.id),
  });
  ...
  const handleRollback = async () => {
    ...
    try {
      const response = await rollbackCharacter(projectId, character.id, rollbackTarget.versionId);
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-slate">查看角色字段变更历史，并回滚到任意版本。当前版本会保留，回滚将创建新版本。</p>
      </div>
